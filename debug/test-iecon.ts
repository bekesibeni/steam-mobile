...
const api = (iface: string, method: string, v = 1) =>
  `https://api.steampowered.com/${iface}/${method}/v${v}/`;
...
const METHODS: Method[] = [
  {
    ...
    http: "GET",
    build: ({ accessToken }) => ({
      url: `${api("IEconService", "GetTradeOffers")}?access_token=${accessToken}&get_sent_offers=1&get_received_offers=1&active_only=1&time_historical_cutoff=0`,
    }),
  },
  {
    name: "GetTradeOffer (invalid id)",
    http: "GET",
    build: ({ accessToken }) => ({
      url: `${api("IEconService", "GetTradeOffer")}?access_token=${accessToken}&tradeofferid=${INVALID_TRADEOFFERID}`,
    }),
  },
  {
    ...
    http: "GET",
    build: ({ accessToken }) => ({
      url: `${api("IEconService", "GetTradeOffersSummary")}?access_token=${accessToken}&time_last_visit=0`,
    }),
  },
  {
    ...
    http: "GET",
    build: ({ accessToken }) => ({
      url: `${api("IEconService", "GetTradeHistory")}?access_token=${accessToken}&max_trades=1&include_total=1`,
    }),
  },
  {
    name: "GetTradeHoldDurations (self)",
    http: "GET",
    build: ({ accessToken, steamID }) => ({
      url: `${api("IEconService", "GetTradeHoldDurations")}?access_token=${accessToken}&steamid_target=${steamID}`,
    }),
  },
  {
    name: "GetTradeStatus (invalid id)",
    http: "GET",
    build: ({ accessToken }) => ({
      url: `${api("IEconService", "GetTradeStatus")}?access_token=${accessToken}&tradeid=${INVALID_TRADEID}`,
    }),
  },
  // ── the two we actually want to settle (invalid id = no real side effect) ──
  {
    name: "CancelTradeOffer (invalid id)",
    http: "POST",
    build: ({ accessToken }) => ({
      url: `${api("IEconService", "CancelTradeOffer")}?access_token=${accessToken}`,
      body: `tradeofferid=${INVALID_TRADEOFFERID}`,
    }),
  },
  {
    name: "DeclineTradeOffer (invalid id)",
    http: "POST",
    build: ({ accessToken }) => ({
      url: `${api("IEconService", "DeclineTradeOffer")}?access_token=${accessToken}`,
      body: `tradeofferid=${INVALID_TRADEOFFERID}`,
    }),
  },
];
    ...
    try {
      ...
      const tag =
        verdict === "inconclusive"
          ? `(HTTP ${res.status} — method not cleanly served, NOT proof of acceptance)`
          : "";
      console.log(
        `${icon} ${m.http.padEnd(4)} ${m.name.padEnd(34)} HTTP ${res.status} eresult=${eresult ?? "-"} ${tag}`,
      );
      if (text) console.log(`              ↳ ${text}`);
    } catch (err) {
      console.log(
        `❓ FETCH FAIL ${m.http.padEnd(4)} ${m.name.padEnd(34)} ${(err as Error).message}`,
      );
    }
  }
