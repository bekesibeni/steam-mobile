 ...
 * using our own Protobuf-ES encode/decode (src/session/tokens.ts).
 *
...
async function main() {
  ...
  if (!existsSync("./bot.refreshtoken"))
    throw new Error("missing ./bot.refreshtoken — run test-iecon.ts first");
  const refreshToken = readFileSync("./bot.refreshtoken", "utf8").trim();

  const rt = decodeJwt(refreshToken);
  console.log(
    `refresh token: sub=${rt?.sub} aud=[${(rt?.aud ?? []).join(",")}] expiresIn=${(secondsUntilExpiry(refreshToken) / 86400).toFixed(1)}d`,
  );

  console.log("\nminting access token via GenerateAccessTokenForApp (hand-rolled protobuf)…");
  const { accessToken, refreshToken: renewed } = await mintAccessToken(
    refreshToken,
    false,
    gotPost,
  );
  ...
  console.log(
    `renewed refresh token returned: ${renewed ? "yes" : "no (expected — we didn't renew)"}`,
  );
